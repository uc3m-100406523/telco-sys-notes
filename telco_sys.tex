...
\subsubsection{Resultant traffic in HU}
...
\paragraph{Traffic calculation}

AAAAAA

\paragraph{Counting method}

In order to obtain the offered traffic in a HU, we must apply a recursive algorithm:

$$
	AB_{total} = AB (N_A \cdot N_B) - \sum_X AX (N_A \cdot N_X) - \sum_{C, Y} CY_{total} + \sum_{C, Y} CY_o (1)
$$
