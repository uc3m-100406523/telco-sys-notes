...
\begin{itemize}
	...
	\item These radio systems are limited by interference. When designing them, a limit for interference is selected, called \textbf{protection rate} ($r_p$).
...
\begin{itemize}
	\item {
		...
		$$
			A_o = \operatorname{B}^{-1} (N - 1, p_B)
		$$
		
		Where $B^{-1}$ stands for Erlang B inverse function.
	...
	\item {
		...
		$$
			N_{cluster} = \frac {W} {\Delta f} = \frac {W} {f_{i} -  f_{i-1}}
		$$
	...
	\item {
		...
		$$
			Q \cdot J \cdot (N - 1) \approxeq C \cdot \frac {S} {J \cdot S_{cell}}
		$$
...
$$
	GoS = \operatorname{P}(W_q > \frac{1}{\mu}) = \operatorname{C}(A_o, c) \cdot e^{- c \cdot \mu \cdot (1-\rho) \cdot t}
$$
