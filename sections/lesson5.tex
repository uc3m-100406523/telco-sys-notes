...
\begin{itemize}
	\item Elevation Angle ($\hat{El}$): From horizontal to satellite direction.
	\item Azimuth Angle ($\hat{A}$): From North to East of horizontal projection of the satellite.
\end{itemize}
...
\begin{itemize}
	...
	\item {
		...
		$$
			\gamma = \cos^{-1} \left( \frac {R} {R + h} \cos(\hat{El}) \right) - \hat{El}
		$$
	}
	\item {
		...
		$$
			d = \sqrt{r_{E}^2 + (R + h)^2 - 2 R (R + h) \cos(\gamma)}
		$$
	}
\end{itemize}
